...
pipeline {
  ...
  stages {
    ...
    stage('Build') {
      ...
      when {
        expression {
          return env.BRANCH_NAME == 'master'|| env.BRANCH_NAME ==~ /(.*\/)?feature-.*/
        }
      }

//       steps {
//         withAWS(role:'jenkins_role', roleAccount:'201523272494', externalId: '', duration: 900, roleSessionName: 'jenkins-session') {
//           sh '''prepare-awscli;
...
//       when {
//         expression {
//           return env.BRANCH_NAME == 'master'|| env.BRANCH_NAME ==~ /(.*\/)?feature-.*/ || env.BRANCH_NAME ==~ /v.*/
//         }
//       }

//       steps {
//         withAWS(role:'jenkins_role', roleAccount:ROK8S_AWS_ACCOUNT_ID, externalId: '', duration: 900, roleSessionName: 'jenkins-session') {
