...
function makeAfterOpProcedure({ from, to }) {
  ...
  begin
    if (tg_op = 'DELETE') then
      ...
      insert into ${to}_subscriptions(short_desc, station_slug, last_updated_utc, filter, mapped_section_fronts, from_station_slug)
      values(new.short_desc, new.station_slug, new.last_updated_utc, new.filter, new.mapped_section_fronts, new.from_station_slug);
      ...
      set short_desc = new.short_desc,
        ...
        filter = new.filter,
        mapped_section_fronts = new.mapped_section_fronts,
