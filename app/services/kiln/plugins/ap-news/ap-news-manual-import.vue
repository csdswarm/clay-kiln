...
<template>
  <div class="ap-media-manual-import">
    ...
    <ui-textbox
      ...
      @change="searchContent"
    ></ui-textbox>
    <ui-progress-circular color="primary" v-show="isLoading" style="width: 100%; height: 44px;"></ui-progress-circular>
    <div v-show="items.length > 0" class="page-list-readout ap-media-manual-import__list">
        <div 
        class="page-list-item ap-media-manual-import__list-item"
        :class="{'ap-media-manual-import__list-item--selected': item.headline === article.headline}"
        @click="selectArticle(item)" :key="idx" v-for="(item, idx) in items" 
        >
          <div class="page-list-item-title">{{ item.headline }}</div>
          <div class="page-list-item-status">
            <a :href="`https://apnews.com/${ item.altids.itemid }`" target="_blank">View</a>
    ...
    <div v-if="article" class="ap-media-manual-import__mapping">
      ...
      <div class="ap-media-manual-import__mapping--buttons">
        ...
        <ui-button @click="importContent" :disabled="!enableImport" :loading="isSubmitting" color="primary">Import</ui-button>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async fetchStationFronts(station){
      ...
      try {
        ...
        console.log(`An error ocurred while fetching section fronts for ${stationSlug}`, err);
      }
    },
    async searchContent() {
