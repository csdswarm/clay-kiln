...
function formatDate(data, locals) {
  if (_get(locals, 'date')) {
    // if locals and locals.date exists, set the gallery date (overriding any date already set)
...
/**
 * get gallery's previously-published data, if it exists
 * note: only grab the data if we're thinking of updating the slug
 * @param  {string} uri
 * @param {object} data
 * @param {object} locals
...
function setSlugAndLock(data, prevData, publishedData) {
  if (initialManualSlug(data, prevData) || manualSlugUpdate(data, prevData)) {
    ...
    // if you've already published the gallery, don't regenerate the slug
