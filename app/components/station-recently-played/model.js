...
module.exports.render = async function (ref, data, locals) {
  ...
  const stationId = locals.stationId ? locals.stationId : locals.station.id,
    ...
    now_playing = getNowPlaying(stationId),
    play_history = radioApi.get(`stations/${stationId}/play_history?event_count=${HISTORY_LIMIT}&before_date=${encodeURIComponent(formattedBeforeDate)}`, null, null, { ttl: radioApi.TTL.MIN * 3, expire: radioApi.TTL.DAY * 7 }).catch(() => {}),
